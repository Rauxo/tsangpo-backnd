...
export const updatePrices = async (req, res) => {
  try {
    ...
    let priceConfig;
    if (latestConfig) {
      if (updates.basePrice !== undefined) latestConfig.basePrice = updates.basePrice;
      if (updates.includedGuests !== undefined) latestConfig.includedGuests = updates.includedGuests;
      if (updates.extraGuestPrice !== undefined) latestConfig.extraGuestPrice = updates.extraGuestPrice;
      if (updates.maxGuests !== undefined) latestConfig.maxGuests = updates.maxGuests;
      ...
      if (updates.addons) {
        ...
        Object.keys(updates.addons).forEach(key => {
          ...
          if (typeof addon === 'object' && addon !== null) {
            ...
            updatedAddons[key] = {
              label: key === 'campsite' ? 'Campsite' :
                     key === 'bonfire' ? 'Bonfire' :
                     key === 'conference' ? 'Conference Hall' : 'Guest Rooms',
              price: addon,
              enabled: true
            };
          }
        ...
        addonTypes.forEach(type => {
          if (!updatedAddons[type]) {
            updatedAddons[type] = {
              label: type === 'campsite' ? 'Campsite' :
                     type === 'bonfire' ? 'Bonfire' :
                     type === 'conference' ? 'Conference Hall' : 'Guest Rooms',
              price: type === 'campsite' ? 4000 :
                     type === 'bonfire' ? 2000 :
                     type === 'conference' ? 5000 : 8000,
              enabled: true
